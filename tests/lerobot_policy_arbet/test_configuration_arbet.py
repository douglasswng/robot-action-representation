...
class TestDefaults:
    def test_default_construction(self):
        ...
        assert cfg.n_token_steps >= 1
        assert cfg.n_token_steps <= cfg.horizon - 1
        assert cfg.n_action_steps >= 1
        assert cfg.n_action_steps <= cfg.n_token_steps
        assert cfg.n_obs_steps >= 1
        assert cfg.bpe_vocab_size >= 8
...
class TestProperties:
    ...
    def test_action_delta_indices_length(self):
        cfg = _make_config(n_obs_steps=2, horizon=16, n_token_steps=8)
