...
class TestModulate:
    ...
    def test_t_first_when_batch_equals_seq_len(self):
        """When B == T, modulate must still broadcast correctly (T-first)."""
        N, D_ = 4, 64
        x = torch.randn(N, N, D_)
        shift = torch.full((N, D_), 1.0)
        scale = torch.zeros(N, D_)
        out = modulate(x, shift, scale)
        assert out.shape == (N, N, D_)
        # Each batch element b should get shift[b] — verify per-batch conditioning.
        for b in range(N):
            torch.testing.assert_close(out[:, b, :], x[:, b, :] + shift[b])
