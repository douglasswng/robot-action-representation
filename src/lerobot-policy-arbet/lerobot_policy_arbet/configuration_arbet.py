...
N_BASE_DIRECTIONS = 8  # Freeman chain-code directions: E, NE, N, NW, W, SW, S, SE
...
class ARBeTConfig(PreTrainedConfig):
    ...
    # ---- Shared (matched with DiTFlow for fair comparison) ----
    ...
    bpe_vocab_size: int = 8192  # Total vocab size after BPE merges (includes 8 base directions + merged tokens)
    ...
    def __post_init__(self):
        ...
        if self.bpe_vocab_size < N_BASE_DIRECTIONS:
            raise ValueError(
                f"`bpe_vocab_size` ({self.bpe_vocab_size}) must be >= {N_BASE_DIRECTIONS} (base direction tokens)."
            )
