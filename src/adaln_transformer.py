...
def modulate(x: torch.Tensor, shift: torch.Tensor, scale: torch.Tensor) -> torch.Tensor:
    ...
    Args:
        x: (T, B, D) or (B, T, D) — the tensor to modulate.
        shift: (B, D) conditioning shift.
        scale: (B, D) conditioning scale.

    Unsqueezes are applied so that conditioning broadcasts over the sequence dim.
    """
    # Expand conditioning to match x layout (works for both T-first and B-first).
    if x.dim() == 3 and shift.dim() == 2:
        if x.shape[0] != shift.shape[0]:
            # x is (T, B, D), shift/scale are (B, D) → unsqueeze dim 0
            shift = shift.unsqueeze(0)
            scale = scale.unsqueeze(0)
        else:
            # x is (B, T, D), shift/scale are (B, D) → unsqueeze dim 1
...
class GateModulation(nn.Module):
    ...
    def forward(self, x: torch.Tensor, c: torch.Tensor) -> torch.Tensor:
        c = self.act(c)
        s = self.scale(c)
        # Broadcast to match x layout
        if x.dim() == 3 and s.dim() == 2:
            s = s.unsqueeze(0) if x.shape[0] != s.shape[0] else s.unsqueeze(1)
        return x * s
