...
class _DiTNoiseNet(nn.Module):
    def __init__(
        ...
        print(f"Number of flow params: {sum(p.numel() for p in self.parameters()) / 1e6:.2f}M")
    ...
    def sample(
        ...
        dt = 1.0 / timesteps
        t_all = torch.arange(timesteps, device=device).float().unsqueeze(0).expand(batch_size, timesteps) / timesteps
...
class DiTFlowModel(nn.Module):
    def __init__(self, config: DiTFlowConfig):
        ...
        if config.training_noise_sampling == "uniform":
            ...
            self.noise_distribution = torch.distributions.TransformedDistribution(beta_dist, [affine_transform])
        else:
    ...
    def conditional_sample(
        ...
        # Sample prior.
        sample = self.velocity_net.sample(global_cond, timesteps=self.num_inference_steps, generator=generator)
        return sample
