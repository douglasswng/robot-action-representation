...
```bash
make check CHECKPOINT=outputs/train/<date>/<run>/checkpoints/<step>  # Weight inspection + rollouts
...
Tests cover `adaln_transformer`, `scribe_tokenizer`, and the ARBeT policy package (configuration, modeling, processor).
...
### Training Pipeline
Training uses `lerobot.scripts.lerobot_train` as the entry point, configured via CLI args in the Makefile. Outputs go to `outputs/train/<date>/<time>_<policy>/` with checkpoints saved at configurable intervals.
...
2. Vision encoding: Crop to 84x84 → `DiffusionRgbEncoder` (ResNet18-based)
3. Policy predicts action sequence (horizon=16, executes n_action_steps=8)
